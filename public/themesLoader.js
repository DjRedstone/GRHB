...
function load(type) {
    $.getJSON("/feed.json", (data) => {
        ...
        if (typeof themes === "string") {
            ...
            $("#article").append(
                `<p class="credit">Publié le ${new Date(article.date).toLocaleDateString(undefined, {
                    weekday: "long",
                    year: "numeric",
                    month: "long",
                    day: "numeric",
            ...
            for (const themePath of themesPaths) {
                ...
                if (title.length >= titleLength) title = title.slice(0, titleLength - 1) + "...";

                $(theme.type === "folder" ? "#grid-list" : "#grid-blog").append(
                    `<a id="${themePath}" class="${theme.type}-grid-case" href="${redirect(themePath)}">${title}</a>`
                );

                if (theme.type === "blog") $(`#${themePath}`).append(`<p class="blog-date">${new Date(theme.date).toLocaleDateString()}</p>`);
            }
        }
